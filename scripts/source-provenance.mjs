...
export function resolveSourceProvenance(repoRoot, env = process.env) {
  ...
  if (env.GITHUB_SHA !== undefined) {
    if (!/^[0-9a-f]{40}$/.test(env.GITHUB_SHA)) {
      throw new SourceProvenanceError('GITHUB_SHA must be a full 40-character lowercase Git SHA')
    }
    if (env.GITHUB_SHA !== sourceCommit) {
      throw new SourceProvenanceError(`GITHUB_SHA does not match checkout HEAD ${sourceCommit}`)
    }
  }
