...
describe('mobile expanded action hierarchy', () => {
  ...
  it('uses the same action footer hierarchy for a blocked plan row', async () => {
    ...
    expect(card?.querySelector('.nrow-card-compose .row-options.comparing')).not.toBeNull()
    expect(card?.querySelector('.nrow-card-compose .compare-toggle')?.textContent).toBe('Hide option details')
    click(card?.querySelector('.compare-toggle'))
    await settle()
    expect(rows()[0]?.querySelector('.row-options.comparing')).toBeNull()
    expect(rows()[0]?.querySelector('.compare-toggle')?.textContent).toBe('Compare options')
