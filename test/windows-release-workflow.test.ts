...
describe('native Windows folder workflow', () => {
  ...
  it('pins actions and proves the runtime, restored folder, PE identities, and launch', () => {
    ...
    expect(workflow).toContain('verify-windows-roundtrip.mjs')
    expect(workflow).toContain('$originalReport.appTreeDigest -ne $report.appTreeDigest')
    expect(workflow).toContain('win32-x64-restored-runtime-verify.json')
    expect(workflow).toContain('dist\\hook-cli.js") selftest')
    expect(workflow).toContain('--node-modules-abi $nodeModulesAbi')
    expect(workflow).toContain('win32-x64-roundtrip.json')
