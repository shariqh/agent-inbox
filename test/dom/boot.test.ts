...
describe('viewer boots against a real DB', () => {
  ...
  it('uses the editorial navigation and plain-language queue vocabulary', async () => {
    ...
    expect(document.querySelector('#needsYouList .queue-tool-group .queue-sort')).not.toBeNull()
    expect(document.querySelector('#needsYouList .queue-tool-group .triage-btn')).not.toBeNull()
    expect(document.querySelectorAll('#needsYouList .nrow-primary')).toHaveLength(2)
    expect(document.querySelectorAll('#needsYouList .nrow-meta')).toHaveLength(2)
