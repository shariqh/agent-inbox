...
describe('Live session state and project identity', () => {
  it('uses project colors while naming working and idle states explicitly', async () => {
    ...
    expect(activeDot?.style.getPropertyValue('--project-dot')).toBe(projectColor('alpha', 'light').dot)
    expect(idleDot?.style.getPropertyValue('--project-dot')).toBe(projectColor('beta', 'light').dot)
    expect(document.getElementById('liveStripDot')?.style.getPropertyValue('--project-dot')).toBe(projectColor('alpha', 'light').dot)
