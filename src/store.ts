...
export function upsertSourceLink(db: Database.Database, link: NewSourceLink): void {
  ...
  db.prepare(
    `INSERT INTO source_links (repo, branch, provider, pr_number, pr_url, pr_title, pr_state, pr_head_sha, pr_draft,
       review_decision, checks, issue_number, issue_url, issue_title, preview_url, preview_environment,
       preview_deployment_id, preview_updated_at, tldr, fetched_at, checked_at, error)
     VALUES (@repo, @branch, @provider, @pr_number, @pr_url, @pr_title, @pr_state, @pr_head_sha, @pr_draft,
       @review_decision, @checks, @issue_number, @issue_url, @issue_title, @preview_url, @preview_environment,
       @preview_deployment_id, @preview_updated_at, @tldr, @now, @now, NULL)
     ON CONFLICT(repo, branch) DO UPDATE SET
       provider = @provider, pr_number = @pr_number, pr_url = @pr_url, pr_title = @pr_title,
       pr_state = @pr_state, pr_head_sha = @pr_head_sha, pr_draft = @pr_draft,
       review_decision = @review_decision, checks = @checks, issue_number = @issue_number,
       issue_url = @issue_url, issue_title = @issue_title, preview_url = @preview_url,
       preview_environment = @preview_environment, preview_deployment_id = @preview_deployment_id,
       preview_updated_at = @preview_updated_at, tldr = @tldr,
...
// A FAILED fetch: touches checked_at and error, and clears preview-only fields.
// The core PR state survives a transient failure, but a preview link is omitted
// until it can be re-proven on a successful check.
