...
bullets, `1.` numbered lists, safe `http://`/`https://` autolinks, and triple-backtick
fenced code or command blocks with an optional short language label. Fenced blocks render
inertly with a Copy control; they are never executed, validated, rewritten, or inferred
from prose. It does not render other Markdown or raw HTML. Keep titles, TL;DRs, notes, and
action fields short and action-first; use that small structure for the reasoning and
history in collapsed `context`. Code- or markup-like fragments outside fences quarantine
the remainder of their paragraph or list item from autolinking; put prose URLs that should
be clickable in a separate paragraph or list item.
