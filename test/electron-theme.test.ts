...
describe('Electron theme bridge wiring', () => {
  ...
  it('pins theme IPC to the active viewer window and native chrome', () => {
    ...
    expect(main).toMatch(/const THEME_BACKGROUND_COLORS = \{\s*light: '#edf0f2',\s*dark: '#090b0d',\s*\}/)
    ...
    expect(main).toContain("THEME_SOURCE_VALUES.has(preference) ? preference : 'dark'")
    expect(main).toMatch(/catch \(err\) \{[\s\S]*nativeTheme\.themeSource = 'dark'[\s\S]*syncThemeChrome\(win\)/)
